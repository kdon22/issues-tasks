...
export function StatusFlowEditor({ flow, workspaceUrl, onBack, onUpdate }: StatusFlowEditorProps) {
  ...
  const { items: statuses, loading, create, update, delete: deleteStatus } = useResource({
    endpoint: `/api/workspaces/${workspaceUrl}/status-flows/${flow.id}/states`,
    cacheKey: `workspace-${workspaceUrl}-flow-${flow.id}-states`,
    mockData: flow.states.map(s => ({
      ...s,
      position: 0,
      type: s.type as State['type']
    })) as State[]
  });

  const handleUpdateFlowInfo = async () => {
    try {
      const response = await fetch(`/api/workspaces/${workspaceUrl}/status-flows/${flow.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ name: flowName, description: flowDescription }),
      });

      if (!response.ok) throw new Error('Failed to update status flow');
      
      toast.success('Status flow updated successfully');
      setEditingFlowInfo(false);
      onUpdate();
    } catch (error) {
      console.error('Error updating status flow:', error);
      toast.error('Failed to update status flow');
    }
  };
  ...
  const handleCreateStatus = async (statusData: { name: string; description: string; color: string; type: State['type'] }) => {
    try {
      await create({
        ...statusData,
        position: statuses.filter(s => s.type === statusData.type).length
      });
  ...
  const handleUpdateStatus = async (status: State, updates: Partial<State>) => {
    try {
      await update(status.id, updates);
  ...
  const handleDragEnd = async (event: DragEndEvent, categoryType: string) => {
    ...
    const categoryStatuses = statuses.filter(s => s.type === categoryType);
    ...
    try {
      await Promise.all(
        reorderedStatuses.map((status, index) => 
          update(status.id, { position: index })
        )
      );
  ...
  const groupedStatuses = STATUS_CATEGORIES.map(category => ({
    category,
    statuses: filteredStatuses.filter(status => status.type === category.key)
  }));
              ...
              <div className="rounded-md border">
                <table className="w-full">
                  ...
                  <SortableContext items={statuses.map(s => s.id)} strategy={verticalListSortingStrategy}>
                    <tbody>
                      {statuses.map((status: State, index) => (
                        <SortableStatusRow
                          key={status.id}
                          status={status}
